...
@interface ZFZSearchBar ()<UITextFieldDelegate>

@property (nonatomic, strong) UIImageView *backgroundImageView;
@property (nonatomic, strong) ZFZSearchTextField *searchTextField;
...
@implementation ZFZSearchBar

- (instancetype)initWithFrame:(CGRect)frame {
    frame = CGRectMake(0, 0, ScreenWidth, 44);
    self = [super initWithFrame:frame];
    //self.backgroundColor = COLOR(243, 243, 243, 1.0);
...
- (void)setIsEditing:(BOOL)isEditing {
    ...
    if (_isEditing) {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            self.rightButton.left = ScreenWidth - 38 - 40;
            self.backgroundImageView.width = ScreenWidth - 20 - 40;
            self.cancelButton.left = ScreenWidth - 40;
        } completion:^(BOOL finished) {
            self.searchTextField.width = ScreenWidth - 20 - 38 - 40;
        }];
        ...
        [UIView animateWithDuration:0.2 animations:^{
            self.top = 0;
            self.searchTextField.left = ScreenWidth * 0.5 - 40;
            self.rightButton.left = ScreenWidth - 38;
            self.backgroundImageView.width = ScreenWidth - 20;
            self.cancelButton.left = ScreenWidth;
        } completion:^(BOOL finished) {
            self.searchTextField.width = ScreenWidth * 0.5 + 20 - 38;
        }];
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.searchTextField.width = ScreenWidth * 0.5 + 20 - 38 - 40;
    }];
    return YES;
}
...
- (UIImageView *)backgroundImageView {
    if (!_backgroundImageView) {
        ...
        _backgroundImageView.frame = CGRectMake(10, 5, ScreenWidth - 20, 44 - 10);
    }
...
- (UITextField *)searchTextField {
    if (!_searchTextField) {
        _searchTextField = [[ZFZSearchTextField alloc] initWithFrame:CGRectMake(ScreenWidth * 0.5 - 40, 0, ScreenWidth * 0.5 + 20 - 38, 44)];
...
//        [_searchTextField addTarget:self action:@selector(textFieldDidChange) forControlEvents:UIControlEventEditingDidEndOnExit];
...
- (UIButton *)rightButton {
    if (!_rightButton) {
        ...
        _rightButton.frame = CGRectMake(ScreenWidth - 38, 8, 28, 28);
    }
    return _rightButton;
}
...
- (UIButton *)cancelButton {
    if (!_cancelButton) {
        ...
        _cancelButton.frame = CGRectMake(ScreenWidth, 0, 40, 44);
        _cancelButton.titleLabel.font = [UIFont systemFontOfSize:16];
        [_cancelButton setTitle:@"取消  " forState:UIControlStateNormal];
        [_cancelButton setTitleColor:COLOR(85, 183, 55, 1.0) forState:UIControlStateNormal];
