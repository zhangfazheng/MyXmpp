...
#import "TabDisplayViewHeader.h"
#import "TabFlowLayout.h"
#import "UIView+Frame.h"

@interface TabDisplayViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation TabDisplayViewController
...
{
    // 初始化标题高度
    _titleHeight = TabTitleScrollViewH;
...
- (void)setUp
{
    ...
    if (_isShowTitleGradient && _titleColorGradientStyle == TabTitleColorGradientStyleRGB) {
...
{
    if (_titleFont == nil) {
        _titleFont = TabDisplayTitleFont;
    }
    return _titleFont;
}
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        // 创建布局
        TabFlowLayout *layout = [[TabFlowLayout alloc] init];
...
- (void)setIsShowTitleScale:(BOOL)isShowTitleScale
{
    if (_isShowUnderLine) {
        // 抛异常
        NSException *excp = [NSException exceptionWithName:@"TabDisplayViewControllerException" reason:@"字体放大效果和角标不能同时使用。" userInfo:nil];
        [excp raise];
    }
...
// 一次性设置所有颜色渐变属性
- (void)setUpTitleGradient:(void (^)(BOOL *, TabTitleColorGradientStyle *, CGFloat *, CGFloat *, CGFloat *, CGFloat *, CGFloat *, CGFloat *))titleGradientBlock
{
...
- (void)viewDidLayoutSubviews
{
    ...
    CGFloat contentY = self.navigationController?NAVIGATION_BAR_HEIGHT + self.haderViewHeight: [UIApplication sharedApplication].statusBarFrame.size.height + self.haderViewHeight;
    CGFloat contentW = ScreenWidth;
    CGFloat contentH = ScreenHeight - contentY;
    // 设置整个内容的尺寸
    ...
    // 设置标题滚动视图frame
    // 计算尺寸
    CGFloat titleW = _titleWidth?_titleWidth:ScreenWidth;
    CGFloat titleH = _titleHeight?_titleHeight:TabTitleScrollViewH;
    ...
    self.contentScrollView.frame = _isfullScreen?CGRectMake(0, self.haderViewHeight, contentW, ScreenHeight) :CGRectMake(0, contentScrollY, contentW, self.contentView.height - contentScrollY);
...
- (void)setUpTitleWidth
{
    ...
    for (NSString *title in titles) {
        ...
        if ([title isKindOfClass:[NSNull class]]) {
            // 抛异常
            NSException *excp = [NSException exceptionWithName:@"TabDisplayViewControllerException" reason:@"没有设置Controller.title属性，应该把子标题保存到对应子控制器中" userInfo:nil];
            [excp raise];
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[TabDisplayTitleLabel alloc] init];
...
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(TabDisplayTitleLabel *)rightLabel leftLabel:(TabDisplayTitleLabel *)leftLabel
{
    ...
    // RGB渐变
    if (_titleColorGradientStyle == TabTitleColorGradientStyleRGB) {
    ...
    // 填充渐变
    if (_titleColorGradientStyle == TabTitleColorGradientStyleFill) {
...
- (void)setUpTitleScaleWithOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    CGFloat scaleTransform = _titleScale?_titleScale:TabTitleTransformScale;
...
- (void)titleClick:(UITapGestureRecognizer *)tap
{
    ...
    if (vc.view) {
        // 发出通知点击标题通知
        [[NSNotificationCenter defaultCenter] postNotificationName:TabDisplayViewClickOrScrollDidFinshNote  object:vc];
        
        // 发出重复点击标题通知
        if (_selIndex == i) {
            [[NSNotificationCenter defaultCenter] postNotificationName:TabDisplayViewRepeatClickTitleNote object:vc];
        }
    }
...
- (void)selectLabel:(UILabel *)label
{
    ...
    for (TabDisplayTitleLabel *labelView in self.titleLabels) {
        ...
        if (_isShowTitleGradient && _titleColorGradientStyle == TabTitleColorGradientStyleFill) {
    ...
    // 标题缩放
    if (_isShowTitleScale && _titleColorGradientStyle == TabTitleColorGradientStyleRGB) {
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    CGFloat underLineH = _underLineH?_underLineH:TabUnderLineH;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 左边按钮
    TabDisplayTitleLabel *leftLabel = self.titleLabels[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    TabDisplayTitleLabel *rightLabel = nil;
