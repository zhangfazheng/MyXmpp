 ...
 **************用法*****************
 ...
 一、导入TabDisplayViewHeader.h
 二、自定义TabDisplayViewController
 三、添加所有子控制器，保存标题在子控制器中
 四、查看TabDisplayViewController头文件，找需要的效果设置
 五、标题被点击或者内容滚动完成，会发出这个通知【"TabDisplayViewClickOrScrollDidFinsh"】，监听这个通知，可以做自己想要做的事情，比如加载数据
...
typedef enum : NSUInteger {
    TabTitleColorGradientStyleRGB,
    TabTitleColorGradientStyleFill,
} TabTitleColorGradientStyle;
 ...
 */
@interface TabDisplayViewController : BaseViewController
...
headView预留高度
 */
@property (nonatomic, assign) CGFloat haderViewHeight;
...
/**
 颜色渐变样式
 */
@property (nonatomic, assign) TabTitleColorGradientStyle titleColorGradientStyle;
...
// 一次性设置所有颜色渐变属性
- (void)setUpTitleGradient:(void(^)(BOOL *isShowTitleGradient,TabTitleColorGradientStyle *titleColorGradientStyle,CGFloat *startR,CGFloat *startG,CGFloat *startB,CGFloat *endR,CGFloat *endG,CGFloat *endB))titleGradientBlock;
