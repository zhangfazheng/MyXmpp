...
+(instancetype)shareManager
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (!isEmptyString(ip) && !isEmptyString(port)) {
            baseUrlString = [NSString stringWithFormat:@"http://%@:%@/Operation",ip,port];
        }
...
- (void)setMyBaseUrl:(NSString *) url{
    [self setValue:[NSURL URLWithString:url] forKey:@"baseURL"];
}
...
{
    //如果用户未登录，不让发送请求
    if (!([LoginUserModel getUserModelInstance].isLogin || [urlString isEqualToString:PhoneLogin] || [urlString isEqualToString:PhoneRegister])) {
...
//    urlString = [urlString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet c]];
